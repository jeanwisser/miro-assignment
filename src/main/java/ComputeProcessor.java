...
public class ComputeProcessor {
    public static SparkSession currentSession = new SparkHelper().StartSession("Compute");
    ...
    public static double computePercentageOfActiveUsers(Dataset<Row> appLoadedRecords, Dataset<Row> registeredRecords) {
        ...
        // We group by user and take the latest registration time in case of duplicates (there is one duplicate in the example)
